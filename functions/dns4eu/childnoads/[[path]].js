const QUAD9_DOH_ENDPOINT = 'https://child-noads.joindns4.eu/dns-query';
...
export async function onRequest(context) {
  try {
    ...
    // 构建目标 URL
    const targetUrl = new URL(QUAD9_DOH_ENDPOINT);
    ...
    return new Response(`Quad9 DNS Proxy Error: ${err.message}`, {
      status: 502,
      headers: { 
...
function handleEdnsParameters(sourceUrl, targetUrl, clientIP) {
  ...
  // 否则不传递edns_client_subnet参数，让Quad9使用默认值
}
...
function createProxyResponse(response) {
  ...
  proxyResponse.headers.set('X-Proxy-Service', 'Cloudflare-Pages-Quad9-DoH');
